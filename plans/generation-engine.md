...
> **Status: ACTIVE — implementation started 2026-07-12.** Design settled; the A–F model is being built as the reviewable step sequence in **§8**. When more lands, a *condensed* section folds into [`architecture.md`](./architecture.md) and the roadmap's [config-object epic](./dungeon-roadmap.md) is decomposed into sub-tasks. Vision context: [`vision.md`](./vision.md).
...
## 8. Implementation steps (approved 2026-07-12)

Small, individually reviewable steps — each lands working code, keeps tsc at the 23 baseline + tests green, uncommitted for review before the next. Maps onto the A–F model (§3).

**Phase 1 — the selection engine (A + B)**
- **Step 1 — `core/select` primitive.** ✅ **Done 2026-07-12** (awaiting review). `src/core/select/index.ts` (3rd `core/` resident): `Tag`/`Row`/`Table` types (labeled-tuple columns), optional `row()` helper, seeded `pickWeighted(table, rng)` = cumulative-weight scan, no array duplication, skips weight≤0, throws on all-zero. 7 Vitest units (`select.test.ts`): determinism, proportional + non-integer weights, zero-weight skip, throw. tsc 23, tests 31.
- **Step 2 — tag-aware selection.** ✅ **Done 2026-07-12** (awaiting review). `pickWeighted(table, rng, ctx?)`, `SelectContext = { tags?, include?, exclude?, boost? }`: hard include (AND) / exclude filters + recovered **soft-boost** (`weight × (1 + boost·matches)`, default boost 1 → `×(matches+1)`); empty ctx = Step 1 behavior (backward compatible). +7 units (14 total in `select.test.ts`). tsc 23, tests 38. Completes engine B.

**Phase 2 — first live consumer (proves the engine)**
- **Step 3 — migrate `core/naming` onto `core/select`.** ✅ **Done 2026-07-12** (awaiting review). Word lists → tagged `Table`s (`structureTable`/`descriptorTable`/`ofTable`); `roomName(tags, rng)` draws via `pickWeighted` — structure noun by **hard include** on the type token (per-type pools + a `"common"` generic pool reproduce the old `STRUCTURES_BY_TYPE`/`STRUCTURES` exactly), water descriptors via **soft-boost** (`WATER_BOOST=4` ≈ old 60% watery bias). Same signature → dungeon call site unchanged; bespoke `pick` retired. +5 units (`naming.test.ts`: determinism, valid output, type-pool containment, generic-pool exclusion, watery bias). tsc 23, tests 43; dungeon suite (name-validity + determinism) still green.

**Phase 3 — context + richer content (C, D)**
- **Step 4 — `GenContext` + namespaced tag taxonomy (`core/model`).** ✅ **Done 2026-07-12** (awaiting review). New `src/core/model/` (4th `core/` resident): `NS` namespaces + `tag(ns,val)` → `"theme:cistern"`/`"water:full"`/`TRAIT_WET`; `GenContext = { seed, tags }` + `rootContext`/`childContext` (parent→child tag accumulation). `core/naming` retagged onto the taxonomy (theme:* include, trait:wet boost). Dungeon: new feature-side `roomTags.ts` (`profileToTags(RoomProfile) → Tag[]`, keeps core generic) + a `rootContext(seed)` → per-room `childContext` → `roomName(ctx.tags, rng)`. **Behavior-preserving** (tags map 1:1 to the same include/boost → names unchanged); adds size/material/feature tags to the context for later consumers. +3 model units. tsc 23, tests 46; dungeon determinism green.
- **Step 5 — `core/text` tracery-lite (D).** ✅ **Done 2026-07-12** (awaiting review). New `src/core/text/` (5th `core/` resident): `expand(template, grammar, rng, ctx?)` — `#symbol#` references drawn from a `Grammar` (weighted `Table` or `string[]`), recursively expanded, seeded, ctx flows tag-boost to weighted pools; `.capitalize`/`.a` modifiers; unknown symbols left literal; depth-guarded. Plus promoted `fill` (the `{key}` slot-fill) + `capitalize` + `article`. `lab/room-description` now imports `fill`/`capitalize` from core (removed its local copies, re-exports `fill` for compat) — behavior unchanged. +8 units (`text.test.ts`). tsc 23, tests 54.

**Phase 4 — structure + scale (E, F, map)**
- **Step 6 — `core/config` registry + Zod (E).** Split into 6a (structural, no dep) + 6b (Zod).
  - **6a — pure config model + registry.** ✅ **Done 2026-07-12** (awaiting review). New `src/core/config/` (6th `core/` resident): `ItemRecord<T>`, `Rule<T,K> = (item, ctx, rng) => value`, `Config<T>`; **pure** `createItem` (runs rules in declaration order → cross-property influence; threads `GenContext` + seeded rng; returns a fresh object, no store) and immutable `rerollProperty`; `createRegistry()` factory (`register`/`get`/`has`/`types`/`create`). The store-free, seedable upgrade of configDungeonGen's jotai `createItm`/`rerollProperty`. +6 units. tsc 23, tests 60.
  - **6b — Zod validation.** ✅ **Done 2026-07-12** (awaiting review). Added **`zod ^4.4.3`** (user-approved; pre-vetted in `architecture.md`). New `src/core/select/schema.ts` (separate from the dep-free engine `index.ts` — zod is opt-in): `RowSchema` (tuple: non-empty value · string[] tags · finite nonnegative weight), `TableSchema`, `validateTable(data) → { ok, table } | { ok:false, errors[] }` for tables loaded as DATA (target F). +8 units. tsc 23, tests 68.
- **Step 7 — adapt the deep vocab (F, ADDITIVE).** ✅ **Done 2026-07-12** (awaiting review). **Non-destructive** per the "legacy is a laboratory" constraint — the original "point the live generators at tables + delete the duplicate engines" is DROPPED; legacy stays byte-for-byte. New `src/content/` zone: `dungeonRoomTables.ts` imports the existing `lists/original/dungeon_rooms` arrays (read-only) and wraps them into tagged `Table`s (theme was in the array NAME → now a `theme:<t>` tag column); `index.ts` = `createTableRegistry()` (validates via 6b `validateTable`) + `contentRegistry` (`room-adjective`/`room-object`/`room-special` by id) + `roomLabel(theme, rng)` consumer (`expand("#adj# #object#", …, { tags:[theme:<t>] })` — adapted vocab BY ID through select+text, the new-engine parallel of `configDungeonGen`'s `pickRandom(tomb_adjective)`). +7 units. tsc 23, tests 75. **git: only new `src/content/*` — zero legacy/snapshot files modified.**
  - *Deferred by constraint:* pointing the LIVE `configDungeonGen`/`character` generators at the new tables, and deleting the duplicate engines — the old engines/vocab coexist as lab specimens.
- **Step 8 — wire the map as config objects.** ✅ **Done 2026-07-12** (awaiting review). New `src/lab/geomorph-dungeon/roomObject.ts`: `RoomObject` type + `roomConfig: Config<RoomObject>` (a `name` rule reading `ctx.tags` via `core/naming`) + `buildRoomObject(profile, root, rng, num)` → an `ItemRecord<RoomObject>` (`type:"room"`, `id:"room-N"`, `parentId:"dungeon"`) whose context = the dungeon root ⊕ the profile's tags (Idea 10). `dungeon.ts` now builds each room as a config object and reads `obj.properties.name`. **Behavior-preserving** (name rule = same `roomName(ctx.tags, rng)` → byte-identical names, proven by an equivalence test). Establishes the north-star pattern (map = view of the generated object tree); a dungeon-level theme flows down via the root context (tested). +4 units. tsc 23, tests 79; dungeon suite green; legacy untouched. *Richer content (themed label / description / contents as more rules; exposing the tree on `DungeonResult`; a dungeon-level theme) = follow-ups.*

*Also advances T1 (seeded selection) and sets up Idea 11 `subRng(seed,path)` (§6) for per-field editing later.*
