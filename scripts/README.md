...
| `generate-dungeon-tiles` | `generateDungeonTiles.ts` | Emits **placeholder** per-material wall-detail "trim" SVGs (`public/dungeonTiles/wall_trim_<1..19>.svg`, `water_trim_<1..19>.svg`; orthogonal edges 1–15, outer-corner nubs 16–19) plus `door_{n,s,e,w}.svg` and `pillar.svg`, and the `src/refactorGeomorphs/geomorph-dungeon/tileConfig.ts` manifest (`TRIM_WALL`, `TRIM_WATER`, `DOOR_TILES`, `PILLAR_TILE`). Colours come from `materials.ts` `TRIM_STYLE`; index scheme lives in `bitmask.ts`. Swap real art by replacing the SVGs (same names) or editing the manifest. |
