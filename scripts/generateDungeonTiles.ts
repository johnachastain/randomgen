...
import { TRIM_STYLE, STAIR_STYLE, PORTAL_STYLE, MATERIAL_COLOR, CAVE_MATERIAL_COLOR } from "../src/lab/geomorph-dungeon/materials.js"
...
// Cave corner tiles (Idea 7 — de-square the walls). A vertex-scan in Page.tsx rounds each CONCAVE
// pocket of the cave silhouette (a floor cell tucked against two walls), filleting it with cave WALL.
// The organic look comes from VARIETY: several corner shapes (arcs of different radii, chamfers,
// rough curves) that Page.tsx selects at random per corner — not one stamped tile. Seeded per
// (orient, variant) so regeneration is stable. `orient` = the floor cell's corner touching the vertex.
const CAVE_WALLC = CAVE_MATERIAL_COLOR[Material.Wall]
// Rounded corners ONLY — a few radii for gentle size variety (no chamfers, no rough curves).
// Apse/alcove variety comes from the wall-bay pass in cave.ts, not from corner-tile styles.
const CAVE_CORNER_SPEC: { R: number }[] = [{ R: 100 }, { R: 78 }, { R: 64 }, { R: 55 }]
type CGeo = { C: [number, number]; P1: [number, number]; P2: [number, number]; sw: 0 | 1 }
const cgeo = (o: Corner, R: number): CGeo =>
  o === "nw" ? { C: [0, 0], P1: [R, 0], P2: [0, R], sw: 0 } :
  o === "ne" ? { C: [V, 0], P1: [V - R, 0], P2: [V, R], sw: 1 } :
  o === "se" ? { C: [V, V], P1: [V, V - R], P2: [V - R, V], sw: 1 } :
              { C: [0, V], P1: [0, V - R], P2: [R, V], sw: 0 }
function caveCornerVariant(o: Corner, spec: (typeof CAVE_CORNER_SPEC)[number]): string {
  const { C, P1, P2, sw } = cgeo(o, spec.R)
  return wrap(`<path d="M ${C[0]},${C[1]} L ${P1[0]},${P1[1]} A ${spec.R} ${spec.R} 0 0 ${sw} ${P2[0]},${P2[1]} Z" fill="${CAVE_WALLC}"/>`)
}
// Cave 1-cell pocket (dome) tile — a smooth half-circle for a floor cell walled on 3 sides. Replaces
// the asymmetrical pointed arch that two independent corner tiles would make there (copies the
// dungeon's alcove half-circle). `open` = the side that connects back to the cave.
const cavePocketTile = (open: Edge): string => {
  const hole = open === "s" ? `M 0,${V} A 50 50 0 0 1 ${V},${V} Z`
    : open === "n" ? `M 0,0 A 50 50 0 0 0 ${V},0 Z`
    : open === "w" ? `M 0,0 A 50 50 0 0 0 0,${V} Z`
    : `M ${V},0 A 50 50 0 0 1 ${V},${V} Z`
  return wrap(`<path fill-rule="evenodd" d="M 0,0 H ${V} V ${V} H 0 Z ${hole}" fill="${CAVE_WALLC}"/>`)
}

mkdirSync(OUT_DIR, { recursive: true })
for (const f of readdirSync(OUT_DIR)) {
  if (/^(corner_|trim_|wall_trim_|water_trim_|cave_|door_|pillar|stairs_|portal_|round_base_|round_corner_|round_trim_|rrcorner_|rrtrim_|alcove_)/.test(f)) rmSync(join(OUT_DIR, f))
}
...
// Cave corner tiles — a VARIETY of concave shapes per orientation (Page.tsx picks one at random).
const caveCornerArrays: Record<string, string[]> = {}
for (const orient of ["nw", "ne", "se", "sw"] as const) {
  const urls: string[] = []
  for (let i = 0; i < CAVE_CORNER_SPEC.length; i++) {
    const file = `cave_corner_${orient}_${i}.svg`
    writeFileSync(join(OUT_DIR, file), caveCornerVariant(orient, CAVE_CORNER_SPEC[i]), "utf-8")
    urls.push(`/dungeonTiles/${file}`)
  }
  caveCornerArrays[orient] = urls
}

// Cave 1-cell pocket (dome) tiles — one per open side.
for (const open of ["n", "s", "e", "w"] as const) {
  writeFileSync(join(OUT_DIR, `cave_pocket_${open}.svg`), cavePocketTile(open), "utf-8")
}
...
// Cave skin manifest (Idea 7) — rounded corner tiles for the organic cave look.
const CAVE_CONFIG_PATH = join(ROOT, "src/lab/geomorph-dungeon/caveTileConfig.ts")
const caveConfig = `// AUTO-GENERATED by scripts/generateDungeonTiles.ts — do not edit manually.
...
export const CAVE_CORNER_VARIANTS: Record<"nw" | "ne" | "se" | "sw", string[]> = {
  nw: ${arrLiteral(caveCornerArrays.nw)},
  ne: ${arrLiteral(caveCornerArrays.ne)},
  se: ${arrLiteral(caveCornerArrays.se)},
  sw: ${arrLiteral(caveCornerArrays.sw)},
}
...
`
writeFileSync(CAVE_CONFIG_PATH, caveConfig, "utf-8")
