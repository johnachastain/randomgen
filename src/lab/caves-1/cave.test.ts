...
describe("generateCave", () => {
  ...
  it("has flat elevation and stairs only as portal stairwells (cave invariants)", () => {
  ...
  it("generates no doors (no Door material in the cave vocabulary)", () => {
    const d = generateCave(30, 24, 99)
    // The dungeon version asserted empty `edges` grids + rect/no-corner rooms. Both are now enforced
    // by CaveResult/RoomInfo, which have no edges/shape/cornerRadius/apses/alcoves to be wrong about.
    // What a TYPE can't catch is a stray material in the grid, so that's what's checked here.
    const valid = new Set([Material.Floor, Material.Wall, Material.Stairs])
    expect(d.grid.flat().every(m => valid.has(m))).toBe(true)
  })
})
