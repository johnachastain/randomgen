import { Material, MaterialGrid, PillarGrid, StairGrid, LevelGrid, RoomInfo, Edge, Portal, PortalKind, DungeonResult } from "./types"
...
export function generateDungeon(cols: number, rows: number): DungeonResult {
  ...
  // 2.5 Side-rooms: small spur rooms off a main room OR a hall, joined by a single 1-wide
  //     opening. Placed BEFORE elevation/doors/water/pillars so they inherit all of them.
  //     Each is fully isolated except the opening ⇒ exactly ONE connection ⇒ a bridge in
  //     the elevation union-find (so it ends up flat, or a ±1 raised/sunken alcove).
  const SDIRS: [number, number][] = [[1, 0], [-1, 0], [0, 1], [0, -1]]
  // The rect + its 1-cell border must be all Wall, except the single `gap` opening cell —
  // this guarantees the side-room touches the rest of the map only through the gap.
  const rectIsolated = (x: number, y: number, w: number, h: number, gapC: number, gapR: number): boolean => {
    if (x < 1 || y < 1 || x + w > cols - 1 || y + h > rows - 1) return false
    for (let r = y - 1; r <= y + h; r++) for (let c = x - 1; c <= x + w; c++) {
      if (c === gapC && r === gapR) continue
      if (!isWall(c, r)) return false
    }
    return true
  }
  // Boundary candidates: an open cell (room edge or hall) with a Wall neighbour to punch
  // through. Shuffled; each is an independent SIDE_ROOM_CHANCE to spawn a spur (so several
  // small alcoves can radiate around a single room; space + isolation limit the rest).
  ...
  for (const [ac, ar, dx, dy] of cands) {
    ...
    const gapC = ac + dx, gapR = ar + dy
    if (!isWall(gapC, gapR)) continue
    const [sw, sh] = Math.random() < TINY_FRACTION ? TINY[randInt(0, 2)] : [randInt(2, 3), randInt(2, 3)]
    let x: number, y: number
    if (dx !== 0) { x = dx > 0 ? gapC + 1 : gapC - sw; y = gapR - randInt(0, sh - 1) }
    else { y = dy > 0 ? gapR + 1 : gapR - sh; x = gapC - randInt(0, sw - 1) }
    if (!rectIsolated(x, y, sw, sh, gapC, gapR)) continue
    for (let r = y; r < y + sh; r++) for (let c = x; c < x + sw; c++) carve(c, r)
    carve(gapC, gapR)                                // the single opening (a natural door / alcove step)
  ...
  const edges = runs.filter(run => unitAt(run.endA) !== -1 && unitAt(run.endB) !== -1)
  ...
  //    `alt` (checkerboard, ~half density), or `perimeter` (outer ring only).
  const pillarOk = (c: number, r: number) => inb(c, r) && (grid[r][c] === Material.Floor || grid[r][c] === Material.Water)
  const PILLAR_PATTERNS = ["all", "alt", "perimeter"] as const
  for (const room of rooms) {
  ...
  return { grid, pillars, rooms, stairs, levels, portals }
}
