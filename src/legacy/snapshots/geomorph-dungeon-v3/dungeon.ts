import { Material, MaterialGrid, PillarGrid, StairGrid, LevelGrid, RoomInfo, RoomShape, Edge, Apse, Portal, PortalKind, EDGE, EdgeKind, EdgeGrids, Corner, DungeonResult } from "./types"
...
export function generateDungeon(cols: number, rows: number): DungeonResult {
  ...
  for (let i = 0; i < targetRooms * 6 && rooms.length < targetRooms; i++) {
    ...
    const room: Room = { x, y, w, h, z: 0, shape, cornerRadius, roundCorners: [], apses: [] }
  ...
  if (rooms.length === 0) {
    ...
    rooms.push({ x: 0, y: 0, w: cols, h: rows, z: 0, shape: "rect", cornerRadius: 0, roundCorners: [], apses: [] })
  }
  ...
  // Ring = each apse cell + its outward Wall neighbours → keep side-rooms/portals a wall-cell clear
  // of every apse too (corridors already avoided them by placement above).
  const apseMargin = new Set<number>()
  for (const [ac, ar] of apseCellList) {
  ...
  // A side-room may not attach off an apse bay, nor onto a room wall that already has an apse.
  const apseBlocked = (c: number, r: number, dx: number, dy: number): boolean => {
    const k = roomAt[r]?.[c]
    if (k === undefined || k < 0) return false
    const rm = rooms[k]
    if (c < rm.x || c >= rm.x + rm.w || r < rm.y || r >= rm.y + rm.h) return true // anchor is an apse cell
    const wall: Edge = dx === 1 ? "e" : dx === -1 ? "w" : dy === 1 ? "s" : "n"
    return rm.apses.some(ap => ap.wall === wall)
  }
  ...
  for (const [ac, ar, dx, dy] of cands) {
    ...
    rooms.push({ x, y, w: sw, h: sh, z: 0, shape: "rect", cornerRadius: 0, roundCorners: [], apses: [] })
  }
