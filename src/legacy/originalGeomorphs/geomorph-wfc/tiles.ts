...
export function buildGrid(collapsed: Map<string, Connects>, cols: number, rows: number): Grid {
  const grid: Grid = []

  for (let row = 0; row < rows; row++) {
    const rowCells: Cell[] = []
    for (let col = 0; col < cols; col++) {
      const connects = collapsed.get(`${col},${row}`) ?? { top: false, right: false, bottom: false, left: false }
...
export function generateGrid(cols: number, rows: number): Grid {
  const collapsed = wfc(cols, rows)
  return buildGrid(collapsed, cols, rows)
}
